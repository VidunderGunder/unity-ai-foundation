...
public class FWCam : MonoBehaviour {
  ...
  void Update() {
    ...
    float objectLength = env.instances.z * env.size + (env.instances.x - 1) * env.margin;
    ...
    float distance = cameraDistance * maxDim / cameraView; // Combined wanted distance from the object
    ...
    // virtualCamera.m_Lens.OrthographicSize = objectSize * screenAspectRatio / 2.0f;
  }
}